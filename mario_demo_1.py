...
def save_plotly_figure(fig, out_path, fmt="png"):
    try:
        ...
        # placeholder
        msg = ["Plotly image export failed", "Install 'kaleido' in requirements.txt and redeploy"]
        W, H = 1200, 800
...
def generate_pdf(fig_radar, fig_keywords, tokens, fig_map, suggestion, selected_detail_place):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 情緒氣泡圖（簡單 snapshot）
        fig_map.write_image(map_path, engine="kaleido", scale=2)
...
if st.button("📑 下載 PDF"):
    try:
        ...
        st.error("產生 PDF 時發生錯誤，請查看後端日誌或本機除錯")
        st.exception(e)
